...
def encrypt(text):
    ...
    salt = os.urandom(8)
    # Standard OpenSSL-compatible AES derivation (like CryptoJS)
    key_iv = PBKDF2(CONFIG["SEC_KEY"], salt, 48, count=1, hmac_hash_module=None)
    key = key_iv[:32]
    iv = key_iv[32:]
    cipher = AES.new(key, AES.MODE_CBC, iv)
    ct_bytes = cipher.encrypt(pad(text.encode('utf-8'), AES.block_size))
    # Prefix with 'Salted__' to match OpenSSL/CryptoJS format
...
if __name__ == "__main__":
    ...
    if all(os.environ.get(r) for r in required):
        if login():
            msgs, nodes = scrape()
    ...
    else: print("SETUP INCOMPLETE: Missing Credentials.")
